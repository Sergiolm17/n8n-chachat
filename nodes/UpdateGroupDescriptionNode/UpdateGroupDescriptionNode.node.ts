...
export class UpdateGroupDescriptionNode implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'CHACHAT Chat Update Group Description Node',
		name: 'updateGroupDescriptionNode',
		group: ['transform'],
		version: 1,
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				const groupId = this.getNodeParameter(
					'groupId',
					itemIndex,
					'120363199451260803@g.us',
				) as string;
				const newDescription = this.getNodeParameter(
					'newDescription',
					itemIndex,
