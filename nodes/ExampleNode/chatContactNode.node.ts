...
export class chatContactNode implements INodeType {
  description:  INodeTypeDescription = {
    displayName: 'CHACHAT Chat Contact Vcard Node',
    name: 'chatContactNode',
    group: ['transform'],
    version: 1,
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
      try {
        ...
        const data = { session: chatSession, contacts: {   displayName: contactName,   contacts: [{ vcard: contactVcard }], },
        };
